...
# 
# seurat_rna[['RNA']] = seurat_rna[['RNA3']]
# seurat_rna[['RNA3']] = NULL
# 
# 
 DefaultAssay(seurat_rna)<- "RNA3"

unique(seurat_rna$SampleID)
...
p <- ggplot(df, aes(y = n_cells, x = reorder(Sample, bar_color), fill = bar_color)) +
...
plot_filename <- paste0(plot_directory, "/barplot_samples_afterfiltering_groupcol", ".pdf")
...
data.object.filter.norm.integrated <- readRDS("Z:/dmclab/Marta/PD/snRNA human putamen/data/rna.combined.snRNA_seq_merged_42_res1.5_annotated_final.rds")

ids<- unique(data.object.filter.norm.integrated$SampleID)
ids_before<- unique(seurat_rna$SampleID)
diff<- setdiff( ids_before,ids)
print(diff)
seurat_rna<-subset(seurat_rna, SampleID %in% ids) 
#check diff agai, should be zero
ids<- unique(data.object.filter.norm.integrated$SampleID)
ids_before<- unique(seurat_rna$SampleID)
diff<- setdiff(ids_before,ids)
diff<- setdiff(ids,ids_before)
print(diff)
data.filt <- SplitObject(seurat_rna, split.by = "SampleID")
saveRDS(seurat_rna, file ="Z:/dmclab/Marta/PD/snRNA human putamen/data/raw_filtered_human.rds")
rm(seurat_rna)
...
```{r}
# SampleIDs from data.object.filter.norm.integrated
sample_ids_integrated <- unique(data.object.filter.norm.integrated$SampleID)

# Check if the SampleIDs are the same
all(sample_ids_from_filt %in% sample_ids_integrated)  # TRUE if all from filt are in integrated
all(sample_ids_integrated %in% sample_ids_from_filt)  # TRUE if all from integrated are in filt

# Check if they match exactly (same elements, same order)
identical(sort(sample_ids_from_filt), sort(sample_ids_integrated))
...
for (id in 1:length(data.filt)) {
  sample_ids_list[[id]] <- unique(data.filt[[id]]$SampleID)
}

# Flatten into a single vector if you want to compare all together
sample_ids_from_filt <- unique(unlist(sample_ids_list))
...
```{r}
unique(data.object.filter.norm.integrated$SampleID)
```
...
head(colnames(data.object.filter.norm.integrated))
```
...
#sanity check to make sure that the spot ids follow the same pattern as in the striatal_clus object i.e <spot_id>_<sample_no.>
head(colnames(temp))==head(colnames(data.object.filter.norm.integrated))

# Step 1: Check if the lengths of the column names are the same
if (length(colnames(temp)) != length(colnames(data.object.filter.norm.integrated))) {
  cat("The objects have different lengths:\n")
  cat("Length of temp:", length(colnames(temp)), "\n")
  cat("Length of seurat:", length(colnames(data.object.filter.norm.integrated)), "\n")
} else {
  cat("The objects have the same length.\n")
}
# Determine the shorter and longer objects
if (length(colnames(temp)) <= length(colnames(data.object.filter.norm.integrated))) {
  shorter <- colnames(temp)
  longer <- colnames(data.object.filter.norm.integrated)
} else {
  shorter <- colnames(data.object.filter.norm.integrated)
  longer <- colnames(temp)
}

# Step 1: Check if all elements in the shorter object are in the longer object
all_in_longer <- all(shorter %in% longer)

if (all_in_longer) {
  cat("All elements in the shorter object are present in the longer object.\n")
} else {
  cat("Some elements in the shorter object are NOT present in the longer object.\n")
  
  # Step 2: Identify which elements are missing
  missing_elements <- setdiff(shorter, longer)
  cat("Missing elements:\n", length(missing_elements))

}


# Step 2: Check if the two objects have the same elements (ignoring order)
if (setequal(colnames(cells.keep), colnames(temp))) {
  cat("The objects contain the same elements, but their order may differ.\n")
} else {
  cat("The objects contain different elements.\n")
  cat("Elements in temp but not in data.object.filter.norm.integrated:\n")
  print(length(setdiff(colnames(cells.keep), colnames(temp))))
  cat("\nElements in data.object.filter.norm.integrated but not in temp:\n")
  print(length(setdiff(colnames(cells.keep), colnames(temp))))
}

# Step 1: Identify mismatched elements
elements_in_temp_not_in_data <- setdiff(colnames(temp), colnames(cells.keep))
elements_in_data_not_in_temp <- setdiff(colnames(cells.keep), colnames(temp))

# Print summary
cat("Number of elements in temp but not in data.object.filter.norm.integrated:", length(elements_in_temp_not_in_data), "\n")
cat("Number of elements in data.object.filter.norm.integrated but not in temp:", length(elements_in_data_not_in_temp), "\n")

# Step 2: Check the clusters of mismatched elements in temp (Active Ident)
if (exists("data.object.filter.norm.integrated")) {
  if (!is.null(Idents(data.object.filter.norm.integrated))) {
    # Check cluster assignments of mismatched elements in temp
    clusters_temp <- Idents(data.object.filter.norm.integrated)[elements_in_temp_not_in_data]
    cat("\nClusters of mismatched elements in temp:\n")
    print(table(clusters_temp, useNA = "ifany"))
  } else {
    cat("The active identity (Idents) is not set for data.filtere.\n")
  }
} else {
  cat("data.filtere object does not exist.\n")
}

# Step 3: Remove mismatched elements from temp
temp <- temp[, !colnames(temp) %in% elements_in_temp_not_in_data]

# Print confirmation
cat("Mismatched elements have been removed from temp.\n")
...
```{r}
cells.keep <- colnames(data.object.filter.norm.integrated)[data.object.filter.norm.integrated@active.ident %in% c("SPNs", "eSPNs")]
