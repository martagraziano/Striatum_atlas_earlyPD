...
df <- as.data.frame(table(rna.combined$SampleID))
colnames(df) <- c('Sample', 'n_cells')

# bar plot of the number of cells in each sample
p <- ggplot(df, aes(y=n_cells, x=reorder(Sample, Sample), fill=Sample_group)) +
...
saveRDS(most_expressed, file = "most_expressed_wS.rds")
```
...
p1 <- VlnPlot(object = rna.combined, features = c("nFeature_RNA"), group.by = "Sample_group", 
    pt.size = 0, log = F)

p2 <- VlnPlot(object = rna.combined, features = c("nCount_RNA"), group.by = "Sample_group",
    pt.size = 0, log = F)
...
p1 <- VlnPlot(object = rna.combined, features = c("nFeature_RNA"), group.by = "Sample_group",fill.by = "Sample_group",
    pt.size = 0, log = T)
...
for(i in 1:length(seurat_rna)){
  seurat_rna[[i]] <- AddMetaData(
  object = seurat_rna[[i]] ,
  metadata = all_doublet_scores[colnames(seurat_rna[[i]])],
  col.name = 'pANN'
)
}
...
features <- SelectIntegrationFeatures(object.list = seurat_rna, nfeatures = 6000)

saveRDS(features, file = "Z:/dmclab/Marta/PD/snRNA human putamen/data/features_used.rds")
seurat_rna_list <- PrepSCTIntegration(object.list = seurat_rna, anchor.features = features)
...
saveRDS(rna.anchors, file = "Z:/dmclab/Marta/PD/snRNA human putamen/data/rna.anchors_human_wSiletti.rds")
rm(rna.anchors)
rna.combined <- IntegrateData(anchorset = rna.anchors)
saveRDS(rna.combined, file = "Z:/dmclab/Marta/PD/snRNA human putamen/data/rna.combined_snRNAseq_human_2.rds")
...
p2 <- DimPlot(object = rna.combined, group.by = "Sample_group", reduction = "umap_after_harmony", alpha=0.6, cols=c("red","blue"))  + ggtitle("sample groups")
...
p2 <- DimPlot(object = rna.combined, group.by = "Sample_group", reduction = "umap_after_harmony_2", alpha=0.6, cols=c( "red","blue"))  + ggtitle("sample groups")
...
p2 <- DimPlot(object = rna.combined, group.by = "Sample_group", reduction = "umap_after_harmony_3", alpha=0.6, cols=c( "red","blue"))  + ggtitle("sample groups")
...
write.csv(wnn_markers, file = "Z:/dmclab/Marta/PD/snRNA human putamen/cluster_markers-HUMAN_snRNA_42_res0.4_annotated.csv", col.names = TRUE, row.names = TRUE)


wnn_markers<- read.csv("Z:/dmclab/Marta/PD/snRNA human putamen/cluster_markers-HUMAN_snRNA_42_res0.4_annotated.csv")
rna.combined<- readRDS(  "Z:/dmclab/Marta/PD/snRNA human putamen/data/rna.combined_res0.4_annotated_2_clinicaldata.rds")
...
write.csv(as.matrix(expression_matrix), "Z:/dmclab/Marta/PD\snRNA human putamen/expression_matrix.csv", row.names = TRUE)
...
mapmycells_results <- read.csv("Z:/dmclab/Marta/PD/snRNA human putamen/new_adata_for_MapMyCells_10xWholeHumanBrain.csv", row.names = 1)
...
plot_filename <- paste0(plot_directory, "/UMAP_ANNOTATED_pc42_res0.4_SIZED_supercluster_name_allen_legend", ".pdf")
...
wnn_markers <- FindAllMarkers(rna.combined, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.2, recorrect_umi = FALSE)
wnn_markers <- wnn_markers[wnn_markers$p_val_adj < 0.05, ]
write.csv(wnn_markers, file = "Z:/dmclab/Marta/PD/snRNA human putamen/new_allen_clustering/cluster_markers-HUMAN_snRNA_42_res1_annotated.csv", col.names = TRUE, row.names = TRUE)
...
saveRDS(rna.combined, file = "Z:/dmclab/Marta/PD/snRNA human putamen/data/rna.combined.snRNA_seq_merged_42_res0.4_2_annotated.rds")
...
plot_age<- ggboxplot(extracted_table, x = "Sample_group", y = "Age",fill="Sample_group",color="black",width=0.3,
  ...
  scale_fill_manual(values=c("gray", "#b4b4daff"))+
  scale_color_manual(values=c("gray", "#b4b4daff"))+
  theme_classic() 
...
# PMI plot (without y-axis)
plot_pmi <- ggplot(metadata_clean, aes(x = factor(1), y = SampleID, color = PMI_group)) +
  geom_point(size = 3) +
  labs(x = "PMI (grouped)") +
  theme_minimal() +
...
plot_braak <- ggplot(metadata_clean, aes(x = factor(1), y = SampleID, label = Braak)) +
  geom_text(size = 4) +
  labs(x = "Braak Stage") +
  theme_classic() +
  theme(axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())

# Combine the three plots side by side using patchwork, keep the axis on the left
combined_plot <- plot_sex +plot_pmi+ plot_braak + plot_layout(ncol = 4)
# Print the combined plot
print(combined_plot)
plot_filename <- paste0(plot_directory, "/plot_braak", ".pdf")
  #png(plot_filename, width = 600, height = 600) 
...
gene_expression <- rna.combined@assays$RNA3@data  # Expression data matrix
...
p2<-DimPlot(object = rna.combined,  reduction = "umap_after_harmony_3",pt.size = 0.5, label = F, label.size = 6, raster=FALSE ,  cols = cl.colors ) + ggtitle("Clusters in combined snRNAseq dataset") 
plot_filename <- paste0(plot_directory, "/umap_DE_COLS", ".pdf")
  #png(plot_filename, width = 1800, height = 600) 
  pdf(plot_filename, width = 11, height = 8)  # Adjust width and height as desired
  print(p2)
  dev.off()  
...
d3<-DotPlot(rna.combined, features =rev(unique(top5_2$gene))) +
 ...
 scale_size(range = c(1,8))+
  coord_flip()
plot_filename <- paste0(plot_directory, "/top_markers_annotated_res1", ".pdf")
  #png(plot_filename, width = 600, height = 600) 
...
HEATMAP<-HEATMAP + theme(
  axis.text = element_text(size = 5),  # Change the size of tick labels
  axis.title = element_text(size = 6)  # Change the size of axis titles
)
plot_filename <- paste0(plot_directory, "/heatmap_top10", ".pdf")
  #png(plot_filename, width = 600, height = 600) 
  pdf(plot_filename, width = 8, height = 13)  # Adjust width and height as desired
  print(HEATMAP)
  dev.off()
...
DefaultAssay(rna.combined) <- "RNA3"
...
for (cl in 1:length(all_clusters)) {
    ...
    Braak_4 <- colnames(rna.combined)[rna.combined@active.ident %in% all_clusters[cl] & rna.combined$Braak %in% "4"]
    Braak_6 <- colnames(rna.combined)[rna.combined@active.ident %in% all_clusters[cl] & rna.combined$Braak %in% "6"]

    if (length(Braak_4) < 3 | length(Braak_6) < 3){
        print("test")
        next
    }
    
    # # Randomly shuffle ground_cells before each random selection
    # shuffled_ct_cells <- sample(ct_cells)
    # 
    # # Randomly select the same number of cells from shuffled ground_cells as in         flight_cells, repeating it multiple times
    num_cells_to_select_b6 <- length(Braak_6)
    selected_braak4_cells <- sample(Braak_4,num_cells_to_select_b6)
    # 
    de_genes_braak[[cl]] <- FindMarkers(rna.combined[["RNA3"]], slot = "data", cells.1 = selected_braak4_cells, cells.2 = Braak_6, test.use = "MAST", latent.vars = "SampleID", min.pct = 0.05, logfc.threshold = 0.2, features = NULL)

    de_genes_braak[[cl]]$Gene <- rownames(de_genes_braak[[cl]])
    
    cl_names <- union(cl_names, all_clusters[cl])
}
names(de_genes_braak) <- cl_names
library(openxlsx)
write.xlsx(de_genes_braak, file = "Z:/dmclab/Marta/PD/snRNA human putamen/Braak4vs_Braak6_DEGs-cluster_res04_2.xlsx")
...
for (cl in 1:length(all_clusters)) {
    ...
    ct_cells <- colnames(rna.combined)[rna.combined@active.ident %in% all_clusters[cl] & rna.combined$Sample_group %in% "Control"]

    if (length(treated_cells) < 3 | length(ct_cells) < 3){
        print("test")
        next
    }
    ...
    # 
    # # Randomly select the same number of cells from shuffled ground_cells as in         flight_cells, repeating it multiple times
    num_cells_to_select <- length(ct_cells)
    selected_treated_cells <- sample(treated_cells,num_cells_to_select)
    # 
    de_genes[[cl]] <- FindMarkers(rna.combined[["RNA"]], slot = "data", cells.1 = selected_treated_cells, cells.2 = ct_cells, test.use = "MAST", latent.vars = "SampleID", min.pct = 0.05, logfc.threshold = 0.2, features = NULL)
...
library(openxlsx)
write.xlsx(de_genes, file = "Z:/dmclab/Marta/PD/snRNA human putamen/PD_vs_wt_DEGs-cluster_res04_2.xlsx")
...
for (clust in clusters){
   #subset<- subset(rna.combined, Sample_group %in% c("Control", "PD"))
   subset_cl<- subset(rna.combined,  idents =clust)
   print(clust)
   keep_rows <- rowSums(subset_cl) > 0
   subset_cluster <- subset_cl[keep_rows, ]
...
plot_directory <- "Z:/dmclab/Marta/PD/snRNA human putamen/plots_2/VolcanoPlots"
...
for (clust in clusters){
  ...
  #df1<- subset(de, cluster!= "Unclassified")
...
plot<-ggplot(result, aes( cluster, reorder(Gene, -avg_log2FC),fill= avg_log2FC))+
 ...
 plot_filename <- paste0(plot_directory, "/Most_sign_genes_braak4vs6_",clust,".pdf")
...
plot1 <- ggplot(result, aes(avg_log2FC, reorder(Gene, -avg_log2FC), color = color_category)) +
  ...
  plot_filename <- paste0(plot_directory, "/Most_sign_genes_dot_braak4vs6_",clust,".pdf")
